...
export default function CourseReader() {
  ...
  const FullscreenMobileLayout = () => {
    ...
    return (
      ...
      >
        {/* Image area - full screen (80%) */}
        <div className='flex-[8] relative overflow-hidden flex items-center justify-center'>
          {activeTopic && (
            <TransformWrapper
              initialScale={1}
              wheel={{ step: 0.1 }}
            ...
            >
              {({ zoomIn, zoomOut, resetTransform }) => (
                <>
                  {/* Exit button - top left, always visible but subtle */}
                  <Button
                    onClick={(e) => {
                      e.stopPropagation();
                      exitFullscreen();
                    ...
                    size='sm'
                    className='absolute top-4 left-4 z-10 bg-black/60 hover:bg-black/80 text-white backdrop-blur-sm'
                  >
                    <Minimize2 className='w-4 h-4' />
                  </Button>

                  {/* Zoom controls - top right, appear with showControls */}
                  <div
          ...
          {/* Current page indicator (always visible but subtle) */}
        ...
        {/* Navigation area - bottom 20% with Previous/Next buttons - ALWAYS VISIBLE */}
        <div className='flex-[2] bg-black/90 border-t border-white/10 flex flex-col justify-center p-4'>
          ...
          {/* Help text - appears with showControls */}
          <div
            className={`text-center text-xs text-white/50 mt-3 transition-all duration-300 ${
              showControls ? 'opacity-100' : 'opacity-0'
            }`}
          >
            Tap screen to {showControls ? 'hide' : 'show'} zoom controls
          </div>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className='flex-1 relative overflow-hidden flex items-center justify-center'>
          ...
          {/* Current page indicator (center bottom, always visible) */}
          <div className='absolute bottom-4 left-1/2 transform -translate-x-1/2'>
            <div className='text-white/70 text-sm bg-black/30 px-3 py-1 rounded-full backdrop-blur-sm'>
              {currentPage + 1} / {totalPages} - {activeTopic?.title}
            </div>
          </div>
        </div>

        {/* Navigation area - bottom with Previous/Next buttons - ALWAYS VISIBLE */}
        <div className='h-16 bg-black/90 border-t border-white/10 flex items-center justify-center px-4'>
          <div className='flex items-center justify-center gap-8 w-full max-w-2xl'>
            <Button
              onClick={() => setCurrentPage((p) => Math.max(0, p - 1))}
              disabled={currentPage === 0}
              className='gap-2 bg-[#22C3B3] hover:bg-[#1AA298] min-w-[150px]'
            >
            ...
            <div className='text-center'>
              ...
              <div
                className={`text-sm text-white/60 mt-1 transition-all duration-300 ${
                  showControls ? 'opacity-100' : 'opacity-0'
                }`}
              >
                Click screen to {showControls ? 'hide' : 'show'} zoom controls
              </div>
            </div>
